# JSON 解构 for uTools

一款完全本地运行的 uTools 插件，用于递归展开被转义的 JSON 对象和数组。

## 功能

- 递归展开对象、数组及其中多层字符串化的 JSON 对象和数组。
- 左右对照显示原始 JSON 与 2 空格缩进的处理结果。
- “解析”和“复制结果”相互独立，只有显式复制时才写入剪贴板。
- 兼容 JSON 字符串内部的非标准单字符转义，例如 URL 中的 `\&`。
- 不限制输入字符长度；递归展开仍保留 100 层安全上限。
- 保留无法解析的嵌套字符串，并以 JSONPath 风格路径显示警告。
- 提示外层解析错误、非对象/数组输入和最大递归深度问题。
- 响应系统深浅主题，窄窗口自动切换为上下布局。
- 零运行时依赖，不联网，不上传、存储或记录输入。

插件优先采用严格 JSON 语法；严格解析失败时，仅兼容字符串内部的非标准单字符转义。插件不支持 JSON5、JavaScript 对象字面量、注释、单引号或尾逗号。字符串形式的数字、布尔值和 `null` 不会转换为原始类型。

## 使用方法

插件提供两种入口：

1. 在 uTools 中输入“JSON 解构”或“递归解析 JSON”打开插件，把内容粘贴到左侧并点击“解析”。
2. 在任意应用复制 JSON 文本，呼出 uTools，选择“递归解构 JSON”；插件会接收所复制的文本并自动解析。
3. 检查右侧结果后，点击“复制结果”写入剪贴板。

如果某段嵌套字符串看起来像对象或数组却不是有效 JSON，其他字段仍会继续处理，警告区会标出对应路径。复制失败时，结果仍会显示，可手动选择复制。

快捷键：

- macOS：`Command+Enter` 执行，`Command+Shift+C` 复制结果。
- Windows/Linux：`Ctrl+Enter` 执行，`Ctrl+Shift+C` 复制结果。

## 本地预览

在项目根目录运行：

```bash
python3 -m http.server 4173
```

然后访问 `http://localhost:4173`。普通浏览器没有 uTools 剪贴板 API，因此可以预览和处理内容；点击“复制结果”时会提示手动复制。

## 运行测试

项目需要 Node.js 18 或更高版本：

```bash
npm test
```

## 接入 uTools

源码开发时，打开 uTools 开发者工具，创建或接入项目并选择项目根目录下的 `plugin.json`。分别验证功能指令入口、复制文本入口、自动解析、显式复制、警告路径、快捷键以及深浅主题。

## 打包 UPXS

先在项目根目录生成干净的发布目录：

```bash
npm run build:release
```

该命令会替换 `release/utools-json-unwrapper`，并只放入 `plugin.json`、页面、样式、运行时 JavaScript 和 PNG Logo，不包含测试、开发文档、SVG 源文件或 `package.json`。`package.json` 中的 `1.0.0` 仅是源码包的参考版本，发布目录本身不嵌入版本号。在 uTools 开发者工具中选择 `release/utools-json-unwrapper/plugin.json`，在打包步骤手动设置并确认 UPXS 版本，再按“打包为离线安装包”流程生成 UPXS。安装该离线包，完成目标平台的基本冒烟测试后再提交发布。

## 发布到应用市场

应用名称、简介、首版说明、截图方案和提交检查项见 [`docs/marketplace.md`](docs/marketplace.md)。发布前重新运行 `npm run build:release`，在 uTools 开发者工具中选择 `release/utools-json-unwrapper` 目录，手动设置并确认本次应用市场版本，准备 Logo、介绍、版本说明、用户手册和截图，填写发布信息并提交审核；只有审核通过后，插件才会进入应用市场。

隐私说明见 [`docs/privacy.md`](docs/privacy.md)。
