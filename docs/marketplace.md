...
- 自动化证据基于代码/构建提交 `e522ade`，验证日期为 2026-08-11：`npm test` 共 43 项通过、0 项失败、0 项跳过；覆盖 12k 层深度序列化与截断语义、`maxDepth` 上限、发布目录删除安全（含项目内目录和符号链接）及警告可访问性。发布构建成功且目录仅包含清单中的 7 个运行时文件；源码与发布版清单入口/Logo 路径有效；运行时与构建 JavaScript 语法检查及 `git diff --check` 均通过。
...
- 已在本地浏览器中截取 2 张草稿截图；应用市场截图清单仍保持未完成，直至补齐真实 uTools 自动复制成功场景并完成整套截图审阅。
